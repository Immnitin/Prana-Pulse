...
export function RegisterPage() {
    ...
    const [formData, setFormData] = useState({
        email: '', password: '', confirmPassword: '', name: '', age: '',
        sex: 'male', phonenumber: '+91 ', location: ''
    });
    ...
    const validateStep1 = () => {
        ...
        if (formData.phonenumber.trim().length < 14) { setError("Please enter a valid 10-digit phone number."); return false; }
    ...
    const validateStep2 = () => {
        ...
        if (!termsAccepted) { setError("You must agree to the terms and privacy policy."); return false; }
        setError(null);
        return true;
    };
    ...
    const renderStep1 = () => (
        <motion.div className="space-y-3" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.4 }}>
    ...
    const renderStep2 = () => (
        <motion.div className="space-y-3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.4 }}>
            ...
            <div>
             ...
             <div>
                <label htmlFor="confirmPassword" className="text-sm font-semibold text-gray-700 block mb-1">Confirm Password</label>
                <div className="relative">
            ...
            <div className="flex items-start">
                <div className="flex items-center h-5">
                    <input id="terms" name="terms" type="checkbox" checked={termsAccepted} onChange={(e) => setTermsAccepted(e.target.checked)} className="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded" />
                </div>
                <div className="ml-3 text-sm">
                    <label htmlFor="terms" className="text-gray-600">I agree to the <a href="#" className="font-medium text-blue-600 hover:underline">Terms of Service</a> and <a href="#" className="font-medium text-blue-600 hover:underline">Privacy Policy</a>.</label>
                </div>
            </div>
        </motion.div>
    );
    ...
    return (
        <PageLayout>
            <div className="w-full min-h-screen lg:grid lg:grid-cols-2 overflow-hidden">
                ...
                {/* Right Column: Form */}
                <div className="flex items-center justify-center py-12 px-4 sm:px-6 bg-gradient-to-br from-gray-50 to-purple-50/30">
                         ...
                         <motion.div className="w-full" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                            ...
                            <div className="pt-2">
                                <div className="flex items-center">
                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold shadow-md transition-all duration-300 ${step >= 1 ? 'bg-gradient-to-r from-blue-500 to-purple-500 text-white' : 'bg-gray-200 text-gray-500'}`}>
                                        {step > 1 ? <CheckIcon className="h-4 w-4" /> : '1'}
                                    </div>
                                    <div className={`flex-1 h-1.5 mx-2 rounded-full transition-all duration-300 ${step >= 2 ? 'bg-gradient-to-r from-purple-500 to-violet-600' : 'bg-gray-200'}`} />
                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold shadow-md transition-all duration-300 ${step >= 2 ? 'bg-gradient-to-r from-purple-500 to-violet-600 text-white' : 'bg-gray-200 text-gray-500'}`}>
                                        2
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl p-5 shadow-lg min-h-[380px]">
                            ...
                            <div className="flex gap-3 pt-1">
                                ...
                                <motion.button type="submit" disabled={isLoading} className="flex-1 flex justify-center items-center px-4 py-2.5 bg-gradient-to-r from-blue-600 via-purple-600 to-violet-600 text-white font-semibold rounded-lg shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300" whileHover={{ scale: 1.02, y: -1 }} whileTap={{ scale: 0.98 }}>
